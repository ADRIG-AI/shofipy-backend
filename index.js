...
} from "./middleware/sessionToken.js";
...
app.get(
  `${apiPrefix}/esg-request-status/:userId/:productId`,
  async (req, res) => {
    try {
...
app.post(`${apiPrefix}/send-esg-request`, async (req, res) => {
  try {
    const { vendorEmail, productName, vendorName, userId, productId } =
      req.body;
    ...
    if (userId && productId) {
      const { error } = await supabase.from("esg_requests").insert({
        user_id: userId,
      ...
      if (error) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to store request in database" });
      }
    }
